...
# Global persistent memory instance — loaded once, used everywhere
_persistent_memory = PersistentMemory()


def _load_agent_memory(n: int = 5) -> List[Dict]:
    """Load the last n entries from persistent cross-session memory."""
    return _persistent_memory.recent(n)


def _save_agent_memory_entry(entry: Dict) -> None:
    """Record an objective execution to persistent cross-session memory."""
    _persistent_memory.record(
...
def run_agent(args) -> None:
    ...
    _persistent_memory.print_startup()
    if recent_memory:
    ...
    # ── Scheduled mode: run scheduler instead of interactive loop ─────
    schedule_str = getattr(args, "schedule", "")
    if schedule_str:
    ...
    while True:
        ...
        if objective.lower() in ("memory", "history", "mem"):
        ...
        # Inject planning hints from past experience
        hints = _persistent_memory.planning_hints(objective)
        if hints:
        ...
        if llm_planner.available:
            ...
            if hints:
                mem_with_hints.insert(0, {
                    "objective": "[PLANNING HINT]",
                    "tool_chain": [],
                    "outcome": hints,
                })
            plan = llm_planner.plan(objective, manifest, mem_with_hints)
            if plan:
                plan_source = "LLM"
        if plan is None:
        ...
        _obj_elapsed = time.time() - _obj_start_time
        memory_entry = {
            "timestamp": time.time(),
            "objective": objective,
            ...
            "outcome": outcome,
            "faults": step_faults,
            "duration_s": round(_obj_elapsed, 2),
...
def _execute_objective(objective: str, registry, llm_planner, corrector,
    ...
    for i, action in enumerate(plan, 1):
        ...
        if not result.success:
            ...
            # Preserve raw output for scheduler formatting
    ...
    # Collect raw step outputs for downstream formatting (scheduler)
    step_results = []
    for a in action_log:
...
if __name__ == "__main__":
    try:
        ...
        # Shutdown picamera2 singleton if running on Pi
        try:
            from aether.core.tool_builder import _picam_instance
